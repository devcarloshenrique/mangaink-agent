> **ARCHIVED** — Esta change foi arquivada como não-aplicável ao estado atual.
> A varredura de `apps/backend/storage/` em 2026-07-16 confirmou **0** `metadata.json`,
> `config.json`, `status.json` ou `images.json` — o `REPO_BACKEND=prisma` já estava
> ativo nos `.env` e `.env.test` desde antes, e o backend em modo Prisma **não gera**
> esses JSONs (apenas logs e binários). A change subsequente `remove-filesystem-backend`
> deletará os próprios leitores desses JSONs. Mantido como registro histórico.
