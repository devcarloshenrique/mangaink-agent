...
2. **Concorrência:** fila de KCC é `concurrency: 1` com lock de 5 min; downloads podem rodar com concorrência maior
...
### 4.1. DownloadOnlyJobData

```ts
interface DownloadOnlyJobData {
  conversionId: string
  jobId: string
  sourceId: string
  chapters: string[]
...
O job de download-only pode ser cancelado via `DELETE /api/conversions/:id` ou `POST /api/conversions/:id/cancel`, que remove o job da fila `download-only` usando `Queue.removeJobs(jobId)`.
...
│   │   │   │   ├── Input (URL)
│   │   │   │   └── Button "Buscar"
│   │   │   ├── SpeechBubble (progresso/erro)
│   │   │   └── ProgressBar (se processing)
│   │   │
│   │   └── [Passo 1: Seleção de Capítulos]
│   │       ├── ComicPanel
│   │       │   ├── Capa thumbnail
│   │       │   ├── Título (font-display)
│   │       │   └── Info: autor, total de capítulos
│   │       ├── ScrollArea com lista de capítulos
│   │       │   └── Checkbox + "Capítulo N" + "X páginas"
│   │       ├── Contador: "N selecionados"
│   │       └── Button "Iniciar download"
```

### 5.2. Estados do Modal

| Estado | Descrição |
|---|---|
| `input` | Campo de URL vazio |
| `inspecting` | Scraping em andamento |
| `ready` | Metadados carregados |
| `failed` | Erro no scraping |
| `submitting` | Enviando POST /api/conversions |
...
<button
  ...
  className="... bg-comic-blue ..."
>
  <Download className="h-4 w-4" /> Adicionar obra
</button>
```
...
- Mensagens do `SpeechBubble` ajustadas para serem neutras
...
3. `overallProgress` usa 0-100% baseado em capítulos processados quando `downloadOnly`
4. Log de `job.finished` mostra mensagem de download concluído
...
│   └── types/download-only.types.ts
├── apps/frontend/src/
│   └── components/biblioteca/AddMangaDialog.tsx
...
│   └── shared/database/repositories/index.ts (se necessário)
├── apps/frontend/src/
│   ├── routes/biblioteca.index.tsx (+ botão + estado)
│   ├── routes/biblioteca.converter.$jobId.tsx (+ downloadOnly UI)
│   ├── hooks/useConversionProgress.ts (+ downloadOnly stages/progress)
│   └── lib/api.ts (CreateConversionBody já cobre)
```
...
| D5 | Modal em vez de página separada | Fluxo rápido, mantém contexto da biblioteca |
...
| `output` opcional pode quebrar validações existentes | Médio | Atualizar Zod schema com `.refine()` condicional |
| Biblioteca mostra download-only sem arquivo de download | Baixo | No progress page, ocultar botão de download para download-only |
...
| Cancelamento de job download-only | Baixo | Reusar mesmo endpoint de cancelamento |
