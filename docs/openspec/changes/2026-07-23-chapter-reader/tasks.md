...
- [ ] 1.1 Adicionar `isDownloaded: boolean` a interface `Chapter` em `apps/backend/src/modules/scraping/types/source.types.ts`.
- [ ] 1.2 Extrair funcao `resolveProvider(sourceId)` do `conversion-job.worker.ts:480-490` para `apps/backend/src/modules/scraping/utils/resolve-provider.ts`. Deve usar singleton de `ProviderResolver` para compartilhar instancias Bottleneck entre proxy e worker.
- [ ] 1.3 Criar tipos `ChapterDownloadData` (job data: `{ sourceId, chapterId }`) e `ChapterDownloadStatus` (status do download: `queued | downloading | ready | failed`) em `apps/backend/src/modules/scraping/types/chapter-download.types.ts`.
- [ ] 1.4 Criar tipo `ChapterManifest` (`{ totalImages: number, urls: string[] }`) para o arquivo `manifest.json` no diretorio de cache.
- [ ] 1.5 Criar `apps/backend/src/modules/scraping/errors/chapter-download.errors.ts` com codigos: `CHAPTER_NOT_FOUND` (404), `PAGE_NOT_FOUND` (404), `INVALID_PAGE_INDEX` (400), `CHAPTER_DOWNLOAD_FAILED` (500). Registrar os status codes no error handler global de `apps/backend/src/shared/server.ts` (extender o `statusMap` existente em linhas 107-124).

## 2. Backend — ChapterImageService (Lightweight)

- [ ] 2.1 Criar `apps/backend/src/modules/scraping/services/chapter-image.service.ts`:
...
- [ ] 3.1 Criar `apps/backend/src/modules/scraping/services/chapter-download-queue.service.ts`:
  - Fila BullMQ `chapter-download` usando `createQueue<ChapterDownloadData>()`
  - Metodos: `enqueue(data)`, `getJob(jobId)`, `close()`
- [ ] 3.2 Criar `apps/backend/src/modules/scraping/workers/chapter-download.worker.ts`:
...
- [ ] 3.3 Criar `apps/backend/src/modules/scraping/services/chapter-download-pubsub.service.ts`:
...
- [ ] 3.4 Criar `apps/backend/src/modules/scraping/services/chapter-download-events.service.ts`:
...
- [ ] 3.5 Registrar worker no bootstrap em `apps/backend/src/shared/server.ts`:
...
- [ ] 4.1 Criar `apps/backend/src/modules/scraping/chapter.routes.ts`:
...
- [ ] 4.2 `POST /api/sources/:sourceId/chapters/:chapterId/download`:
...
- [ ] 4.3 `GET /api/sources/:sourceId/chapters/:chapterId/download`:
...
- [ ] 4.4 `GET /api/sources/:sourceId/chapters/:chapterId/download/events`:
...
- [ ] 4.5 `GET /api/sources/:sourceId/chapters/:chapterId/images/:index` (publico, sem JWT):
...
- [ ] 4.6 Estender `GET /api/conversions/source/inspect/:sourceId`:
...
- [ ] 4.7.1 Modificar `apps/backend/src/modules/conversion/services/image-downloader.service.ts`:
...
- [ ] 4.7.2 Teste: adicionar cenario em `image-downloader.service.test.ts` (teste existente) que verifica que `manifest.json` e escrito apos o download (cache miss path) e que **nao** e escrito no cache hit path

## 5. Backend — Testes

- [ ] 5.1 `chapter-image.service.test.ts`: download de imagens (mock provider), cache hit/miss, validacao de magic bytes, escrita/leitura de manifest, `isCached()`.
- [ ] 5.2 `chapter-download.worker.test.ts`: fluxo completo com `MockScrapingProvider`, mock do `RedisPubSubService`, mock do `SourceCacheRepository`. Verificar que `manifest.json` e imagens sao escritos, eventos Pub/Sub emitidos.
- [ ] 5.3 `chapter-download-events.service.test.ts`: subscribe/unsubscribe, replay de eventos do journal, cleanup no close.
- [ ] 5.3.1 `chapter-download-pubsub.service.test.ts`: publicacao no canal `chapter-download:{sourceId}:{chapterId}`, subscribe recebe mensagens corretas, unsubscribe limpa conexao.
- [ ] 5.4 `get-source.use-case.test.ts`: adicionar cenario que verifica `isDownloaded` no array `chapters` do response.
- [ ] 5.5 `resolve-provider.test.ts`: resolucao via sourceId com `SourceCacheRepository.load()` mockado, fallback quando source nao existe.
- [ ] 5.6 E2E `POST .../download`: retorna 202 com `jobId`, valida se job foi enfileirado na fila `chapter-download`.
- [ ] 5.7 E2E `GET .../download`: retorna status correto (`queued`, `downloading`, `ready`, `failed`).
- [ ] 5.8 E2E `GET .../download/events`: SSE emite `progress` durante download e `completed` ao finalizar.
- [ ] 5.9 E2E `GET .../images/:index`:
...
- [ ] 6.1 Adicionar tipos em `apps/frontend/src/types/`:
...
- [ ] 6.2 Adicionar funcoes no API client (`apps/frontend/src/lib/api.ts` ou novo `chaptersApi`):
...
- [ ] 7.1 Criar `apps/frontend/src/hooks/useChapterDownload.ts`:
...
- [ ] 7.2 Criar `apps/frontend/src/hooks/useChapterPages.ts`:
...
- [ ] 8.1 Atualizar `apps/frontend/src/components/biblioteca/TabCapitulos.tsx`:
...
- [ ] 8.2 Criar `apps/frontend/src/components/biblioteca/DownloadChapterDialog.tsx`:
...
- [ ] 8.3 Criar `apps/frontend/src/components/reader/ChapterReader.tsx`:
...
- [ ] 9.1 Criar `apps/frontend/src/routes/biblioteca.reader-chapter.$sourceId.tsx`:
...
- [ ] 9.2 Atualizar `apps/frontend/src/routes/biblioteca.$sourceId.tsx`:
...
- [ ] 10.4 `useChapterDownload.test.ts`:
...
- [ ] 10.5 `useChapterPages.test.ts`:
  - Gera URLs corretas para cada indice de pagina
  - Array vazio quando `totalPages=0`

## 11. Integracao e Verificacao

- [ ] 11.1 Remover `MOCK_CACHED_CHAPTERS` e referencias mockadas da aba Capitulos.
- [ ] 11.2 Verificar que `routeTree.gen.ts` e regenerado corretamente apos nova rota.
- [ ] 11.3 `pnpm lint` — zero erros ESLint em todos os pacotes.
- [ ] 11.4 `pnpm test` — todos os testes (existentes + novos) passam.
- [ ] 11.5 `pnpm build` — frontend e backend compilam sem erros TypeScript.
- [ ] 11.6 Teste manual end-to-end:
