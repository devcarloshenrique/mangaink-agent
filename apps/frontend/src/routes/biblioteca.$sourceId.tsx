...
function statusLabel(s: ConversionStatus): string {
  const map: Record<ConversionStatus, string> = {
    queued: "Na fila", processing: "Convertendo", completed: "Concluído",
    failed: "Erro", cancelled: "Cancelado", partial: "Parcial",
  };
  return map[s] ?? s;
}
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-4xl px-4 py-10">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm font-medium opacity-70 mt-1">
              {conversions.length} conversão(ões) • {hasActive && <Loader2 className="h-3 w-3 inline animate-spin" />}
            </p>
          </div>
        ...
        {isLoading && (
          <div className="text-center py-16"><Loader2 className="h-8 w-8 animate-spin mx-auto text-comic-blue" /></div>
        )}

        {!isLoading && conversions.length === 0 && (
          <SpeechBubble variant="yellow" tail="left" className="max-w-md">Nenhuma conversão encontrada.</SpeechBubble>
        )}
          ...
          <div className="space-y-4">
            {conversions.map((conv) => (
              <Link
                ...
                to={conv.status === "completed"
                  ? "/biblioteca/reader/$conversionId"
                  : "/biblioteca/converter/$jobId"
                }
                params={conv.status === "completed"
                  ? { conversionId: conv.conversionId }
                  : { jobId: conv.conversionId }
                }
                className="block"
              >
                <ComicPanel bg="card" padding="md" tilt="none" className="relative group hover:-translate-y-0.5 transition-transform cursor-pointer">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-display text-lg truncate">{conv.title || conv.conversionId}</h3>
                        <span className={cn("text-xs font-medium px-2 py-0.5 border-2 border-ink rounded-full", conv.status === "completed" ? "bg-comic-blue text-white" : "bg-muted")}>{statusLabel(conv.status)}</span>
                      </div>
                      <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground flex-wrap">
                        <span className="flex items-center gap-1"><FileText className="h-3.5 w-3.5" /> {conv.completedJobs}/{conv.totalJobs} jobs</span>
                    ...
                    <div className="shrink-0 flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                      ...
                      {(conv.status === "queued" || conv.status === "processing") && (
                        <Button size="sm" variant="outline" className="border-[2px] border-ink shadow-comic-sm h-8 px-2" onClick={() => cancel(conv.conversionId)} title="Cancelar">
                          <XCircle className="h-4 w-4" />
                        </Button>
                      )}
                      <Button size="sm" variant="outline" className="border-[2px] border-ink shadow-comic-sm h-8 px-2" onClick={() => remove(conv.conversionId)} title="Remover">
