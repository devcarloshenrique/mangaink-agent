...
function deriveStages(
  ...
  const allDone = apiJobs.length > 0 && apiJobs.every((j) => ['completed', 'failed', 'cancelled'].includes(j))
  ...
  const downloadProgress = totalChapters > 0 ? Math.round((processedChapters / totalChapters) * 100) : 0

  const conversionDone = allDone
  const convActive = isConversionActive && !conversionDone
  const conversionProgress = totalJobs > 0
    ? Math.min(100, Math.round((completedJobs * 100) / totalJobs + currentJobProgress / totalJobs))
    : isConversionActive ? currentJobProgress : 0
