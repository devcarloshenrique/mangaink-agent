...
import { ReaderChapterIndex, type ReaderIndexItem } from "@/components/reader/ReaderChapterIndex";
...
export interface ReaderCoreProps {
  ...
  onBack?: () => void;
  mangaMode?: boolean;
  navItems?: ReaderIndexItem[];
  currentNavId?: string;
  prevNavId?: string | null;
  nextNavId?: string | null;
  onNavigateNavId?: (id: string) => void;
  navItemLabel?: string;
  onRetry?: () => void;
  isLoading?: boolean;
  hasError?: boolean;
...
export function ReaderCore({
  ...
  const prevNavIdRef = useRef(currentNavId);
  ...
  const effectiveTotal = totalPages > 0 ? totalPages : pageUrls.length;

  // Prefetch das próximas 2 páginas para virada de página instantânea (0ms)
  useEffect(() => {
    if (pageUrls.length === 0) return;
    const next1 = currentPage + 1;
    const next2 = currentPage + 2;
    if (next1 < pageUrls.length) {
      const img1 = new Image();
      img1.src = pageUrls[next1];
    }
    if (next2 < pageUrls.length) {
      const img2 = new Image();
      img2.src = pageUrls[next2];
    }
  }, [currentPage, pageUrls]);

  // Reset de página quando muda o item de navegação (capítulo ou volume)
  useEffect(() => {
    const isItemChange = prevNavIdRef.current !== currentNavId;
  ...
  // Loading geral inicial / sem páginas
      ...
      <div className="fixed inset-0 z-50 bg-reader-bg flex flex-col items-center justify-center gap-5">
        ...
        {hasError ? (
          <div className="flex flex-col items-center gap-3 mt-2">
            <p className="text-sm text-reader-muted">{errorMessage}</p>
            {onRetry && (
              <button
                onClick={onRetry}
          ...
          <p className="text-xs text-reader-muted/60">{transitionMessage}</p>
        )}
      </div>
    );
  }
    ...
    <div className="fixed inset-0 z-50 bg-reader-bg select-none">
      ...
      {showProgressBar &&
        ...
        (progressPosition === "bottom" ? (
            ...
            <div className="absolute bottom-0 left-0 right-0 h-[2px] group-hover:h-9 transition-all duration-200 ease-in-out overflow-hidden">
              ...
              <div className="absolute inset-0 z-10 bg-reader-bg/90 backdrop-blur-sm border-t border-reader-border opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <div className="flex items-center gap-3 px-3 h-full">
                  ...
                  <div
                    ref={barRef}
                    className="flex-1 flex gap-1 h-3 items-center"
                    ...
                    {Array.from({ length: effectiveTotal }, (_, i) => (
                      <div
                        ...
                        className={`flex-1 h-full rounded-sm transition-colors ${
                          i <= currentPage ? "bg-reader-accent" : "bg-reader-surface"
                        }`}
                      />
                    ))}
                  </div>
          ...
          <div
            className="fixed top-0 left-0 right-0 z-20 h-[50px] cursor-pointer group"
            onMouseMove={handleHitboxMouseMove}
            onMouseLeave={handleHitboxMouseLeave}
            onClick={handleHitboxClick}
          >
            ...
            <div className="absolute top-0 left-0 right-0 h-[2px] group-hover:h-9 transition-all duration-200 ease-in-out overflow-hidden">
              <div className="absolute inset-0 pointer-events-none">
                <div className="absolute inset-0 bg-reader-surface" />
                <div
                  className="absolute inset-y-0 left-0 bg-reader-accent"
              ...
              <div className="absolute inset-0 z-10 bg-reader-bg/90 backdrop-blur-sm border-b border-reader-border opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                <div className="flex items-center gap-3 px-3 h-full">
                  <span className="text-reader-muted text-xs tabular-nums min-w-[1.5rem] text-right">
                    {currentPage + 1}
                  </span>
      ...
      {showProgressBar && progressStyle === "circular" && effectiveTotal > 0 && (
        <div
          ...
          <div className="relative w-10 h-10 flex items-center justify-center">
            <svg className="w-10 h-10 -rotate-90" viewBox="0 0 36 36">
              <circle
                ...
                r="15.9155"
                fill="none"
                className="stroke-reader-surface"
                strokeWidth="2.5"
              />
              <circle
                cx="18"
                ...
                r="15.9155"
                fill="none"
                className="stroke-reader-accent transition-all duration-150"
                strokeWidth="2.5"
                strokeDasharray={`${progressPct}, 100`}
                strokeLinecap="round"
              />
            </svg>
            <span className="absolute text-[10px] tabular-nums text-reader-muted font-medium">
              {currentPage + 1}
            </span>
          </div>
        </div>
          ...
          <>
            {!imageLoaded && (
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-5 pointer-events-none">
                <BookOpen
                  className="w-8 h-8 text-reader-muted/50 animate-pulse"
                  strokeWidth={1.25}
                />
                <p className="text-[11px] uppercase tracking-[0.3em] text-reader-muted/70">
                  Mangaink
                </p>
                <p className="text-xs text-reader-muted/60">{transitionMessage}</p>
              </div>
      ...
      <ReaderFloatingMenu
        showUI={showUI}
        hasPrevChapter={!!prevNavId}
        hasNextChapter={!!nextNavId}
        onPrevChapter={() => onNavigateNavId?.(prevNavId!)}
        onNextChapter={() => onNavigateNavId?.(nextNavId!)}
      ...
      <ReaderChapterIndex
        ...
        onSelectItem={(id) => onNavigateNavId?.(id)}
      />
    </div>
  );
}
