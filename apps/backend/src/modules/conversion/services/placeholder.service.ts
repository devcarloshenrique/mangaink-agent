...
export class PlaceholderService {
  ...
  private getDeviceResolution(deviceId: string): { width: number; height: number } {
    ...
    if (!device) return { width: PlaceholderService.DEFAULT_W, height: PlaceholderService.DEFAULT_H }

    const parts = device.resolution.split('x')
    const w = parseInt(parts[0], 10)
    const h = parseInt(parts[1], 10)

    if (isNaN(w) || isNaN(h)) return { width: PlaceholderService.DEFAULT_W, height: PlaceholderService.DEFAULT_H }
